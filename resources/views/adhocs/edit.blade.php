                ...
                <div class="md:col-span-2 p-4 rounded-2xl bg-amber-50/70 border border-amber-200 space-y-2">
                    <div class="flex items-center justify-between">
                        <label class="block text-xs font-bold text-amber-900 uppercase tracking-wider">
                            ...
                            Supervisor Penanggung Jawab ACC / Approval
                        </label>
                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full {{ $adhoc->isPendingApproval() ? 'bg-amber-200 text-amber-900' : ($adhoc->isApproved() ? 'bg-emerald-200 text-emerald-900' : 'bg-rose-200 text-rose-900') }}">
                            Status Saat Ini: {{ $adhoc->txtApprovalStatus ?: 'Approved' }}
                        </span>
                    </div>
                    <select name="intSupervisor_ID" class="w-full px-3.5 py-2.5 rounded-xl border border-amber-300 text-sm focus:border-[#006838] focus:ring-2 focus:ring-[#006838]/20 outline-none bg-white font-medium">
                        <option value="">-- Otomatis Pilih Supervisor Berdasarkan Sub-Departemen --</option>
                    ...
                    <div class="p-2.5 rounded-xl bg-white border border-amber-200 text-xs text-amber-950 mt-1">
                        <span class="font-bold block text-[10px] uppercase text-amber-800">Catatan / Alasan Review Supervisor:</span>
                        <em>"{{ $adhoc->txtApprovalNotes }}"</em>
                    </div>
                    @endif
                </div>
