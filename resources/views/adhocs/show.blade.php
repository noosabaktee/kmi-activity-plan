...
<div class="space-y-6">
    ...
    @php
        ...
        $approvalStatus = $adhoc->txtApprovalStatus ?: 'Approved';
        $approvalClasses = match($approvalStatus) {
            'Pending Approval' => 'bg-amber-100 text-amber-900 border-amber-300',
            'Approved' => 'bg-emerald-100 text-emerald-900 border-emerald-300',
            'Rejected' => 'bg-rose-100 text-rose-900 border-rose-300',
            default => 'bg-gray-100 text-gray-800 border-gray-200',
        };
        $approvalIcon = match($approvalStatus) {
            'Pending Approval' => 'fa-solid fa-hourglass-half',
    ...
    <div class="bg-white p-6 rounded-3xl border border-[#DDE5DD] shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div class="space-y-2">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black border {{ $approvalClasses }}">
                    <i class="{{ $approvalIcon }} text-[10px]"></i>
                    <span>{{ $approvalStatus === 'Pending Approval' ? 'Menunggu ACC Supervisor' : ($approvalStatus === 'Approved' ? 'Telah di-ACC' : 'Ditolak / Revisi') }}</span>
                </span>
            ...
            <p class="text-xs text-gray-500 m-0 flex flex-wrap items-center gap-2">
                ...
                &bull;
                <span>Supervisor: <strong class="text-gray-900">{{ $adhoc->supervisor?->txtEmployeeName ?? 'Belum Ditugaskan' }}</strong></span>
                &bull;
    ...
    <!-- Approval Status / Action Banner -->
    @if ($adhoc->isPendingApproval())
        @if ($authUser && $authUser->canApproveAdHoc($adhoc))
        <!-- Action Card for Supervisor / Approver -->
        <div class="bg-amber-50/90 border-2 border-amber-300 rounded-3xl p-6 shadow-sm space-y-4">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-2xl bg-amber-500 text-white flex items-center justify-center text-lg shadow-sm shrink-0">
                        <i class="fa-solid fa-bell"></i>
                    </div>
                    <div>
                        <h3 class="text-sm font-black text-amber-950 m-0">Pengajuan Ad Hoc Membutuhkan Persetujuan (ACC) Anda</h3>
                        <p class="text-xs text-amber-800 m-0">Inisiatif ini diajukan oleh <strong>{{ $adhoc->user?->txtEmployeeName ?? 'Employee' }}</strong> dan memerlukan persetujuan sebelum dapat dijalankan.</p>
                    </div>
                </div>
                <span class="px-3 py-1 rounded-full bg-amber-200 text-amber-900 text-xs font-black uppercase tracking-wider shrink-0">
                    Menunggu Tindakan Anda
                </span>
            </div>

            <!-- Approval Action Form -->
            <div class="bg-white p-4 rounded-2xl border border-amber-200 space-y-3">
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider">
                    Catatan / Instruksi Supervisor (Opsional untuk ACC, Wajib jika Menolak/Minta Revisi):
                </label>
                <textarea id="supervisorFeedback" rows="2" placeholder="Tuliskan arahan, instruksi teknis, atau catatan revisi..." class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-xs focus:border-[#006838] focus:ring-2 focus:ring-[#006838]/20 outline-none"></textarea>

                <div class="flex flex-wrap items-center justify-end gap-2.5 pt-2 border-t border-gray-100">
                    <!-- Reject Form -->
                    <form action="{{ route('adhocs.reject', $adhoc) }}" method="POST" id="rejectForm" onsubmit="return handleReject(event)">
                        @csrf
                        <input type="hidden" name="txtApprovalNotes" id="rejectNotesHidden">
                        <button type="submit" class="px-4 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs border border-rose-300 transition flex items-center gap-2 cursor-pointer shadow-2xs">
                    ...
                    <!-- Approve Form -->
                    <form action="{{ route('adhocs.approve', $adhoc) }}" method="POST" id="approveForm" onsubmit="handleApprove()">
                        @csrf
                        <input type="hidden" name="txtApprovalNotes" id="approveNotesHidden">
                        <button type="submit" class="px-5 py-2.5 rounded-xl bg-[#006838] hover:bg-[#004d29] text-white font-black text-xs shadow-md transition flex items-center gap-2 cursor-pointer" style="background-color: #006838; color: #ffffff;">
                            <i class="fa-solid fa-check text-white"></i>
                            <span>ACC / Setujui Inisiatif Ad Hoc</span>
                        </button>
                    </form>
                </div>
            </div>
        ...
        @else
        <!-- Information Banner for Requester / Viewer -->
        <div class="bg-amber-50 border border-amber-200 rounded-3xl p-5 flex items-center gap-3.5">
            <div class="w-10 h-10 rounded-2xl bg-amber-500 text-white flex items-center justify-center text-lg shrink-0 shadow-2xs">
                <i class="fa-solid fa-hourglass-half"></i>
            </div>
            <div class="space-y-0.5">
                <h4 class="text-xs font-black uppercase tracking-wider text-amber-900 m-0">Menunggu Persetujuan Supervisor</h4>
                <p class="text-xs text-amber-800 m-0 leading-relaxed">
                    Inisiatif Ad Hoc ini telah diajukan kepada <strong>{{ $adhoc->supervisor?->txtEmployeeName ?? 'Supervisor' }}</strong>. Notifikasi pesan WhatsApp telah dikirimkan ke supervisor untuk segera ditinjau dan di-ACC.
                </p>
            </div>
        </div>
        @endif
    @elseif ($adhoc->isApproved())
        <div class="bg-emerald-50 border border-emerald-200 rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-2xl bg-[#006838] text-white flex items-center justify-center text-lg shrink-0 shadow-2xs">
                    <i class="fa-solid fa-circle-check text-white"></i>
                </div>
                <div class="space-y-0.5">
                    <h4 class="text-xs font-black uppercase tracking-wider text-emerald-900 m-0">Inisiatif Telah Disetujui (ACC) oleh Supervisor</h4>
                    <p class="text-xs text-emerald-800 m-0">
                        Disetujui oleh <strong>{{ $adhoc->approvedBy?->txtEmployeeName ?? $adhoc->supervisor?->txtEmployeeName ?? 'Supervisor' }}</strong>
                        @if ($adhoc->dtmApprovedAt)
                            pada {{ $adhoc->dtmApprovedAt->format('d M Y H:i') }}
                        @endif
                        @if ($adhoc->txtApprovalNotes)
                            &bull; Catatan: <em>"{{ $adhoc->txtApprovalNotes }}"</em>
                        @endif
                    </p>
                </div>
            </div>
            <span class="px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-[11px] font-extrabold shrink-0 border border-emerald-200">
                Disetujui (Siap Dijalankan)
            </span>
        </div>
    @elseif ($adhoc->isRejected())
        <div class="bg-rose-50 border border-rose-200 rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-2xl bg-rose-600 text-white flex items-center justify-center text-lg shrink-0 shadow-2xs">
                    <i class="fa-solid fa-triangle-exclamation text-white"></i>
                </div>
                <div class="space-y-0.5">
                    <h4 class="text-xs font-black uppercase tracking-wider text-rose-900 m-0">Inisiatif Ditolak / Memerlukan Revisi</h4>
                    <p class="text-xs text-rose-800 m-0">
                        Ditinjau oleh <strong>{{ $adhoc->approvedBy?->txtEmployeeName ?? 'Supervisor' }}</strong>
                        @if ($adhoc->dtmApprovedAt)
                            pada {{ $adhoc->dtmApprovedAt->format('d M Y H:i') }}
                        @endif
                        &bull; Alasan / Catatan Revisi: <strong class="text-rose-950">"{{ $adhoc->txtApprovalNotes }}"</strong>
                    </p>
                </div>
            </div>
            <a href="{{ route('adhocs.edit', $adhoc) }}" class="px-3.5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs transition shrink-0 no-underline shadow-xs" style="background-color: #e11d48; color: #ffffff;">
                <i class="fa-solid fa-pen-to-square mr-1"></i> Edit & Perbaiki
            </a>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white p-6 rounded-3xl border border-[#DDE5DD] shadow-xs space-y-3 text-xs">
                <h3 class="text-xs font-black text-gray-400 uppercase tracking-wider m-0">Informasi Teknis & Persetujuan</h3>
                <div class="space-y-2 divide-y divide-gray-100 text-[11px]">
                    ...
                    <div class="flex justify-between py-1.5">
                        <span class="text-gray-500">Supervisor Peninjau:</span>
                        <span class="font-bold text-gray-900">{{ $adhoc->supervisor?->txtEmployeeName ?? 'Belum Ditugaskan' }}</span>
                    </div>
                    <div class="flex justify-between py-1.5">
                        <span class="text-gray-500">Status Persetujuan:</span>
                        <span class="font-bold {{ $approvalStatus === 'Approved' ? 'text-emerald-700' : ($approvalStatus === 'Rejected' ? 'text-rose-700' : 'text-amber-700') }}">
                            {{ $approvalStatus === 'Pending Approval' ? 'Menunggu ACC' : ($approvalStatus === 'Approved' ? 'Disetujui (ACC)' : 'Ditolak / Revisi') }}
                        </span>
                    </div>
                    @if ($adhoc->approvedBy)
                    <div class="flex justify-between py-1.5">
                        <span class="text-gray-500">Ditinjau Oleh:</span>
                        <span class="font-bold text-gray-900">{{ $adhoc->approvedBy->txtEmployeeName }}</span>
                    </div>
                    @endif
                    @if ($adhoc->dtmApprovedAt)
                    <div class="flex justify-between py-1.5">
                        <span class="text-gray-500">Waktu Peninjauan:</span>
                        <span class="font-bold text-gray-900">{{ $adhoc->dtmApprovedAt->format('d M Y H:i') }}</span>
                    </div>
                    @endif
                    <div class="flex justify-between py-1.5">
...
function handleApprove() {
    ...
    const hiddenInput = document.getElementById('approveNotesHidden');
    if (hiddenInput) {
        hiddenInput.value = feedback;
    }
    return true;
}
...
function handleReject(event) {
    ...
    if (!feedback) {
        ...
        alert('Mohon masukkan catatan atau alasan revisi/penolakan sebelum menolak.');
        document.getElementById('supervisorFeedback')?.focus();
        return false;
    }
    if (!confirm('Apakah Anda yakin ingin menolak pengajuan ad hoc ini atau meminta revisi?')) {
        event.preventDefault();
        return false;
    }
    const hiddenInput = document.getElementById('rejectNotesHidden');
    if (hiddenInput) {
        hiddenInput.value = feedback;
    }
    return true;
