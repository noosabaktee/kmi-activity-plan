                ...
                <div class="bg-white p-6 rounded-3xl border border-[#DDE5DD] shadow-xs space-y-3 text-xs">
                    ...
                    <div class="space-y-2 divide-y divide-gray-100 text-[11px]">
                        ...
                        <div class="flex justify-between py-1.5">
                            ...
                            <span class="font-bold text-gray-900">{{ $adhoc->supervisor?->txtEmployeeName ?? 'Otomatis' }}</span>
                        </div>
                        <div class="flex justify-between py-1.5">
                            <span class="text-gray-500">Status Persetujuan:</span>
                            <span class="font-bold {{ $adhoc->isPendingApproval() ? 'text-amber-700' : ($adhoc->isApproved() ? 'text-emerald-700' : 'text-rose-700') }}">
                                {{ $adhoc->txtApprovalStatus ?: 'Approved' }}
                            </span>
                        </div>
                        @if ($adhoc->approvedBy)
                        <div class="flex justify-between py-1.5">
                            <span class="text-gray-500">Di-ACC / Direview:</span>
...
<script>
    function handleApprove() {
        const notes = document.getElementById('supervisorFeedback')?.value || '';
    ...
    function handleReject(e) {
        const notes = document.getElementById('supervisorFeedback')?.value.trim() || '';
        if (!notes) {
            e.preventDefault();
            alert('Mohon tuliskan alasan penolakan / catatan revisi pada kolom feedback supervisor sebelum menolak.');
            document.getElementById('supervisorFeedback')?.focus();
            return false;
        }
        if (!confirm('Apakah Anda yakin ingin menolak / mengembalikan inisiatif Ad Hoc ini untuk revisi?')) {
            e.preventDefault();
            return false;
        }
