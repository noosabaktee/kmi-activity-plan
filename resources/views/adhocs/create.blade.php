@extends('layouts.app', [
'title' => 'Buat Inisiatif Ad Hoc - KMI Activity Plan',
'pageTitle' => 'BUAT INISIATIF AD HOC BARU',
'pageSubtitle' => '<span>Penanganan Sasaran Khusus & Kondisi Sementara</span> &bull; <span>Action Plan & Tim Pelaksana</span>',
])
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-extrabold text-gray-900 tracking-tight m-0">Formulir Inisiatif Ad Hoc</h2>
            <p class="text-xs text-gray-500 m-0">Rencanakan tugas ad hoc khusus dengan sasaran tunggal dan rentang waktu sementara.</p>
        </div>
        <a href="{{ route('adhocs.index') }}" class="px-3.5 py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-xs transition flex items-center gap-2 no-underline">
    ...
    <form action="{{ route('adhocs.store') }}" method="POST" class="space-y-6" id="adHocForm">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="txtProjectName" value="{{ old('txtProjectName') }}" required
                        ...
                        placeholder="Contoh: Penanganan Lonjakan Beban Chiller Line 2">
                </div>
                ...
                <div>
                    ...
                    <select name="txtAdHocCategory" required class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-sm focus:border-teal-700 outline-none">
                        ...
                        <option value="{{ $cat }}" {{ old('txtAdHocCategory') == $cat ? 'selected' : '' }}>
                            {{ $cat }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-2">
                        ...
                        <label class="border-2 rounded-xl p-2 text-center cursor-pointer transition flex flex-col items-center justify-center gap-1 {{ old('txtPriority', 'Medium') == $key ? $u['active'] . ' font-black' : 'border-gray-200 text-gray-600 hover:border-teal-600' }}">
                            <input type="radio" name="txtPriority" value="{{ $key }}" {{ old('txtPriority', 'Medium') == $key ? 'checked' : '' }} class="hidden" onchange="updateUrgencyStyle(this)">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="txtSpecialGoal" rows="2" required
                        class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-xs focus:border-teal-700 outline-none"
                        placeholder="Tuliskan sasaran khusus spesifik yang harus tercapai saat penanganan ad hoc ini selesai...">{{ old('txtSpecialGoal') }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="txtDescription" rows="2"
                        class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-xs focus:border-teal-700 outline-none"
                        placeholder="Kondisi lapangan, akar penyebab awal, atau permintaan manajemen yang melatarbelakangi dibentuknya ad hoc ini...">{{ old('txtDescription') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-[#DDE5DD] shadow-xs space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="dtmProjectStartDate" id="startDateInput" value="{{ old('dtmProjectStartDate', date('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="dtmProjectEndDate" id="endDateInput" value="{{ old('dtmProjectEndDate', date('Y-m-d', strtotime('+14 days'))) }}" required
                        class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-sm focus:border-teal-700 outline-none" onchange="calculateDuration()">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-[11px] text-gray-500 mb-1.5">Pilih karyawan yang ditugaskan sebagai PIC utama dan anggota gugus tugas penanganan ad hoc ini (live search multi-select):</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="txtDeliverable" value="{{ old('txtDeliverable') }}"
                        ...
                        placeholder="Contoh: Laporan RCA & Modifikasi Setting Parameter Selesai">
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Status Awal</label>
                    <select name="txtStatus" class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-sm focus:border-teal-700 outline-none">
                        <option value="In Progress" {{ old('txtStatus') == 'In Progress' ? 'selected' : '' }}>In Progress</option>
                        <option value="Under Review" {{ old('txtStatus') == 'Under Review' ? 'selected' : '' }}>Under Review</option>
                        <option value="Completed" {{ old('txtStatus') == 'Completed' ? 'selected' : '' }}>Completed</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="txtTargetSkalaGrade" rows="3"
                        class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-xs focus:border-teal-700 outline-none font-mono"
                        placeholder="1. Masalah teratasi 100%&#10;2. Tidak ada reoccurrence selama 7 hari operasi&#10;3. Dokumentasi perbaikan disetujui Department Head">{{ old('txtTargetSkalaGrade', "1. Masalah teratasi 100%\n2. Tidak ada reoccurrence selama 7 hari operasi\n3. Laporan selesai diverifikasi") }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Bobot Kontribusi Kinerja (%)</label>
                    <input type="number" step="0.5" min="0" max="100" name="floatWeight" value="{{ old('floatWeight', 10) }}"
                        class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-sm focus:border-teal-700 outline-none"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left" id="actionStepsTable">
                    ...
                    <tbody class="divide-y divide-gray-100" id="actionStepsBody">
                        <tr class="action-row">
                            <td class="p-2"><input type="text" name="stages[0][step]" value="Identifikasi & Analisis Masalah / Requirement" class="w-full px-2.5 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                        ...
                        <tr class="action-row">
                            <td class="p-2"><input type="text" name="stages[1][step]" value="Eksekusi Tindakan Perbaikan / Pelaksanaan Solusi" class="w-full px-2.5 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="date" name="stages[1][start]" value="{{ date('Y-m-d', strtotime('+4 days')) }}" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="date" name="stages[1][end]" value="{{ date('Y-m-d', strtotime('+8 days')) }}" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="number" step="1" name="stages[1][plan]" value="40" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs text-center"></td>
                            <td class="p-2"><input type="number" step="1" name="stages[1][actual]" value="0" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs text-center"></td>
                            <td class="p-2 text-center"><button type="button" onclick="this.closest('tr').remove()" class="text-red-400 hover:text-red-600 p-1"><i class="fa-solid fa-trash"></i></button></td>
                        </tr>
                        <tr class="action-row">
                            <td class="p-2"><input type="text" name="stages[2][step]" value="Verifikasi & Pengujian Hasil di Lapangan" class="w-full px-2.5 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="date" name="stages[2][start]" value="{{ date('Y-m-d', strtotime('+9 days')) }}" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="date" name="stages[2][end]" value="{{ date('Y-m-d', strtotime('+11 days')) }}" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="number" step="1" name="stages[2][plan]" value="20" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs text-center"></td>
                            <td class="p-2"><input type="number" step="1" name="stages[2][actual]" value="0" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs text-center"></td>
                            <td class="p-2 text-center"><button type="button" onclick="this.closest('tr').remove()" class="text-red-400 hover:text-red-600 p-1"><i class="fa-solid fa-trash"></i></button></td>
                        </tr>
                        <tr class="action-row">
                            <td class="p-2"><input type="text" name="stages[3][step]" value="Dokumentasi Laporan & Handover / Penutupan" class="w-full px-2.5 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="date" name="stages[3][start]" value="{{ date('Y-m-d', strtotime('+12 days')) }}" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            <td class="p-2"><input type="date" name="stages[3][end]" value="{{ date('Y-m-d', strtotime('+14 days')) }}" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs"></td>
                            ...
                            <td class="p-2"><input type="number" step="1" name="stages[3][actual]" value="0" class="w-full px-2 py-1.5 rounded-lg border border-gray-300 text-xs text-center"></td>
                            <td class="p-2 text-center"><button type="button" onclick="this.closest('tr').remove()" class="text-red-400 hover:text-red-600 p-1"><i class="fa-solid fa-trash"></i></button></td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
            ...
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-[#006838] hover:bg-[#004d29] text-white font-bold text-sm shadow-md hover:shadow-lg transition flex items-center gap-2 cursor-pointer" style="background-color: #006838; color: #ffffff;">
                ...
                <span>Simpan Inisiatif Ad Hoc</span>
                <i class="fa-solid fa-paper-plane text-white"></i>
                <span>Ajukan Inisiatif Ad Hoc ke Supervisor</span>
            </button>
        </div>
    </form>
</div>
...
@php
$authUserId = $authUser->intUser_ID;
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const initialSelected = [currentAuthUserId];
