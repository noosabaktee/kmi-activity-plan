    ...
    <form action="{{ route('projects.store') }}" method="POST" class="space-y-6" id="projectForm">
        ...
        <div class="bg-white p-6 rounded-3xl border border-[#DDE5DD] shadow-xs space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2 hidden" id="multiAssignmentNotice">
...
<script>
    ...
    // Sub projects in-memory state
    let subProjectsList = [{
            name: 'Vibe Coding',
            weight: 35,
            start_date: '{{ date("Y-01-05") }}',
            end_date: '{{ date("Y-05-30") }}',
            deliverable: 'Pipeline otomatis coding agent',
            assignments: [currentAuthUserId],
            stages: [{
                    step: 'Initiation & Setup Environment',
                    start: '{{ date("Y-01-05") }}',
                    end: '{{ date("Y-02-15") }}',
                    plan: 25,
                    actual: 25
        ...
        },
        {
            ...
            end_date: '{{ date("Y-08-30") }}',
            deliverable: 'Assistant agent chatbot',
            assignments: [currentAuthUserId],
            stages: [{
                    step: 'Requirement & Prompt Design',
                    start: '{{ date("Y-02-01") }}',
                    end: '{{ date("Y-04-15") }}',
                    plan: 30,
                    actual: 30
                ...
                {
                    step: 'User Acceptance & Pilot Run',
                    start: '{{ date("Y-07-16") }}',
                    end: '{{ date("Y-08-30") }}',
                    plan: 25,
                    actual: 0
        ...
        },
        {
            ...
            deliverable: 'Automated data entry bots',
            assignments: [currentAuthUserId],
            stages: [{
                    step: 'Process Analysis & Mapping',
                    start: '{{ date("Y-06-01") }}',
                    end: '{{ date("Y-07-31") }}',
                    plan: 25,
                    actual: 20
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const initialSingleAssignments = @json(old('assignments', [$authUserId]));
